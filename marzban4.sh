...
update_core_command() {
    ...
    xray_executable_path="XRAY_EXECUTABLE_PATH=\"/var/lib/marzban4/xray-core/xray\""
...
services:
  marzban:
    ...
    network_mode: host
    volumes:
      - /var/lib/marzban4:/var/lib/marzban4
      - /var/lib/marzban4/logs:/var/lib/marzban-node
    depends_on:
      mariadb:
  ...
  mariadb:
    ...
    volumes:
      - /var/lib/marzban4/mysql:/var/lib/mysql
    healthcheck:
...
EOF
        ...
        sed -i 's~^\(SQLALCHEMY_DATABASE_URL = "sqlite:////var/lib/marzban4/db.sqlite3"\)~#\1~' "$APP_DIR/.env"
        ...
        sed -i 's~\(XRAY_JSON = \).*~\1"/var/lib/marzban4/xray_config.json"~' "$APP_DIR/.env"
...
EOF
	...
	sed -i 's/^UVICORN_PORT *= *8000/UVICORN_PORT = 8004/' "$APP_DIR/.env"
        sed -i 's/^# \(XRAY_JSON = .*\)$/\1/' "$APP_DIR/.env"
        sed -i 's/^# \(SQLALCHEMY_DATABASE_URL = .*\)$/\1/' "$APP_DIR/.env"
        sed -i 's~\(XRAY_JSON = \).*~\1"/var/lib/marzban4/xray_config.json"~' "$APP_DIR/.env"
        sed -i 's~\(SQLALCHEMY_DATABASE_URL = \).*~\1"sqlite:////var/lib/marzban4/db.sqlite3"~' "$APP_DIR/.env"
